    ...
    let mat_internal_diag_partial = mat_internal_diag_m_1;
        ...
        meta.create_gate("partial rounds", move |meta| {
            ...
            let first_inputs: Vec<_> = (0..WIDTH)
                ...
                .collect();
            constraints.push(pow_7(first_inputs[0].clone()) - sbox_a.clone());
    ...
    fn permute(
        ...
        layouter.assign_region(
            ...
            |mut region| {
                ...
                let mut state = Pow7State::load(&mut region, config, initial_state)?;
...
impl<F: Field, const WIDTH: usize> Pow7State<F, WIDTH> {
    fn first_layer<const RATE: usize>(
        ...
        next_state.map(|state| Pow7State(state.try_into().unwrap()))
    }
...
mod tests {
    ...
    fn hash_test_vectors() {
        for tv in crate::poseidon2::primitives::test_vectors_7t3::fp::hash() {
            let message = [
