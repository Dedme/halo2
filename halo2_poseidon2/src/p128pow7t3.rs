...
mod tests {
    ...
    fn test_basic_pow7_permutation() {
        ...
        // Test with Fp
        {
            let (round_constants, mds, _) = <super::P128Pow7T3 as Spec<Fp, 3, 2>>::constants();
            let mut state_fp = [Fp::from(0), Fp::from(1), Fp::from(2)];
            permute::<Fp, super::P128Pow7T3, 3, 2>(&mut state_fp, &mds, &round_constants);
            // Just verify it doesn't panic and produces non-zero output
            assert!(state_fp[0] != Fp::from(0) || state_fp[1] != Fp::from(0) || state_fp[2] != Fp::from(0));
        }
        
        // Test with Fq
        {
            let (round_constants, mds, _) = <super::P128Pow7T3 as Spec<Fq, 3, 2>>::constants();
            let mut state_fq = [Fq::from(0), Fq::from(1), Fq::from(2)];
            permute::<Fq, super::P128Pow7T3, 3, 2>(&mut state_fq, &mds, &round_constants);
            // Just verify it doesn't panic and produces non-zero output
            assert!(state_fq[0] != Fq::from(0) || state_fq[1] != Fq::from(0) || state_fq[2] != Fq::from(0));
        }
    }
    ...
    fn test_pow7_hash() {
        ...
        let result_fp = Hash::<_, super::P128Pow7T3, ConstantLength<2>, 3, 2>::init().hash(message_fp);
        assert!(result_fp != Fp::from(0));
        
        let message_fq = [Fq::from(6), Fq::from(42)];
        let result_fq = Hash::<_, super::P128Pow7T3, ConstantLength<2>, 3, 2>::init().hash(message_fq);
